...
where $\mathbf{X}_i$ is the covariate vector for individual $i$ and $\boldsymbol\beta$ is a vector of regression covariates.
...
In other words, the odds changes by a factor of $\exp(.2322)\approx.79$ as temperature increases by one degree.
