...
Under these assumptions, $\overline{X}-\overline{Y}$ is approximately normal and the test statistic
$$Z=\frac{\overline{X}-\overline{Y}-(\mu_1-\mu_2)}{\sqrt{\frac{S_1^2}{m}+\frac{S_2^2}{n}}}$$
has approximately a standard normal distribution when $H_0$ is true.

## Example: Two-sample $t$-test

When the samples are small, but the population distributions are approximately normal, the standardized test statistic 
...
# an example where y is not independent of the gene
null_y <- rnorm(300) 
...
For this reason it is common to perform a correction to the hypothesis tests when many hypothesis tests are conducted.
