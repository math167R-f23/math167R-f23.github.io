...
We can use this simulation code to learn a number of things about random walks:
...
walk_randomly_2d <- function(start, n_steps) {
  if (n_steps <= 1) {
    return(start)
  }
  x <- rbind(start, walk_randomly_2d(start + take_step_2d(), n_steps - 1))
  rownames(x) <- NULL
  return(x)
}
n_steps <- 100
coords <- walk_randomly_2d(c(0, 0), n_steps)
coords
```
