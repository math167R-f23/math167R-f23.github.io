...
## B. Writing a simulation function
...
5. Write a function `random_walk()` that takes as input a single numeric $n$ and simulates $n$ steps of a one-dimensional random walk. The output should be a length vector of length $n+1$ starting with 0 where the $(i+1)$th entry represents the location of the random walker at time $t=i$. For example, `random_walk(5)` may return the vector $(0, 1, 2, 1, 2)$.

6. Use your function to generate a random walk of 100 steps and generate a line graph with $t=0,1,\ldots, 100$ on the x-axis and $x_0,\ldots, x_{100}$ on the y-axis.